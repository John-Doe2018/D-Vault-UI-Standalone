	...
	mfb
			.run([
					...
					function($templateCache) {
						...
						$templateCache
								.put(
										'ng-mfb-button-default.tpl.html',
										'<li>'
							...
							function($timeout) {
								return {
									...
									link : function(scope, elem, attrs) {
										...
										/**
										 * Check if we're on a touch-enabled
										 * device. Requires Modernizr to run,
										...
										/**
										 * Convert the toggling method to
										 * 'click'. This is used when 'hover' is
										 * selected by the user but a touch
										 * device is enabled.
										 */
										function useClick() {
										 ...
										 */
										function flipState() {
											scope.menuState = scope.menuState === openState ? closedState
													: openState;
										}

										/**
										 * Set the state to user-defined value.
										 * Fallback to closed if no value is
										...
										/**
										 * If on touch device AND 'hover' method
										 * is selected: wait for the digest to
										 * perform and then change hover to
										 * click.
										 */
										if (_isTouchDevice()
												&& _isHoverActive()) {
